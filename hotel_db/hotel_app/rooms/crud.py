...
async def get_rooms(db: AsyncSession, skip: int, limit: int):
    rooms = (await db.execute(
        ...
        .limit(limit)
        .offset(skip)
        .order_by(Room.room_num)
    )).all()
    res = [{"room_num": room_num, "category_name": category_name}
           for room_num, category_name in rooms]
    return res
...
async def create_room(db: AsyncSession, room: schemas.RoomCreate):
    await get_cateogry_by_id(db, room.category_id)
...
async def update_room(db: AsyncSession,
    ...
    room = {key: value for key, value in updated_room.model_dump().items()
            if value != None}
    if room == {}:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST)
    if not await is_room_unique_by_room_nuum(db, updated_room.room_num) and room_num != updated_room.room_num:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Room must be unique")
