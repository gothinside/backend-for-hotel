...
from ..models import Service, Booking, client_booking, booking_service, Client, Room
from sqlalchemy import select, insert, func, and_, update, delete, or_
...
async def validate_date(db: AsyncSession, room_num: int, join_date: datetime, out_date: datetime):
    result = (await db.execute(
        select(Booking)
        .where(Booking.room_num == room_num)
        .where((and_(Booking.join_date >= join_date,
                    Booking.out_date <= out_date)))
    ...
    result1 = (await db.execute(
        select(Booking)
        .where(Booking.room_num == room_num)
        .where(and_(Booking.join_date <= out_date,
                    Booking.out_date >= out_date)))).scalars().all()
    
    result2 = (await db.execute(
        select(Booking)
        .where(Booking.room_num == room_num)
        .where(and_(Booking.join_date <= join_date,
                    Booking.out_date >= join_date)))).scalars().all()
